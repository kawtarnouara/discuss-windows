const {app, BrowserWindow, Menu, session, ipcMain, TouchBar, nativeImage, Tray} = require('electron');
...
async function createWindow() {
    ...
    win = new BrowserWindow({
        ...
        icon: `file://${__dirname}/icons/private_k9o_icon.icns`,
    ...
    splash = new BrowserWindow({width: 300, height: 300, backgroundColor: '#eeeeee', frame: false, alwaysOnTop: true});
    ...
    const templateFull = [{
        ...
        submenu: [
            {label: "Recharger", accelerator: "CmdOrCtrl+R", click: function () {
                    app.setBadgeCount(0);
                    win.reload();
                    // win.loadURL(`file://${__dirname}/discuss/index.html`);
            }},
            {type: "separator"},
...
exports.setBadge = (count) => app.setBadgeCount(count);
// AUTO UPDATER
...
ipcMain.on('online-status-changed', (event, status) => {
    ...
    if (status === 'online' && currentStatus !== 'online') {
        ...
        splash.loadURL(`file://${__dirname}/assets/splash_private.html?connection=1`);
        // win.loadURL(`file://${__dirname}/discuss/index.html`);
        win.loadURL(`https://app.private-discuss.com`);
